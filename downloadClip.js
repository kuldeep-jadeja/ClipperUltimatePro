...
export default async function handler(req, res) {
    ...
    try {
        ...
        // Try to fetch download URL if missing
        if (!mp4) {
            try {
        ...
        // Fetch the video file from CDN or Twitch
        const cdnRes = await fetch(mp4);
        if (!cdnRes.ok) {
            return res.status(cdnRes.status).json({ error: `CDN responded ${cdnRes.status}` });
        }

        // Set download headers
        const filename = `${clip.streamerName.replace(/\s+/g, "_")}_${clipId}.mp4`;
        res.setHeader("Content-Type", "video/mp4");
        res.setHeader("Content-Disposition", `attachment; filename="${filename}"`);

        // Stream video file directly to client
        cdnRes.body.pipe(res);
    } catch (err) {
